...
void desfazimento()
{
   ...
   if ( !obrir_FicDez_rb() )
   {
      ...
      reg_header->codi_transaccio = 2;
      reg_header->nid = reg_header->nid++;
      reg_header->poscar = reg_header->poscar + 4;
...
void llegir_ficvar( void )
{
   if (((!iOrdresObertes) && (tipus_registre->a == 'I')) ||
       ...
       (((!iOrdresObertes) && (iContestacio != 0) && (tipus_registre->a == 'D'))) ||
       (((!iOrdresObertes) && (tipus_registre->a == 'D') && ((iOpe == 112) || (iOpe == 114)))))
   {
      iOrdresObertes = 1;
      generar_ficord();
   }

   if ((tipus_registre->a == 'P') && (!iLlegitP))
   {
      iLlegitP = 1;
      fputc('P', lpszOrdres);
      escriure_salt_registre();
   ...
   else
   {
      for (i=0; i < (tamany_byte->a - 1); i++)
      {
         ...
         if (((iContestacio != 0) && (tipus_registre->a == 'D')) ||
            ((tipus_registre->a == 'D') && ((iOpe == 112) || (iOpe == 114))))
            fputc(buffer[i], lpszOrdres);
      }
   }
   if ((tipus_registre->a == 'I') || (tipus_registre->a == 'P') ||
       (tipus_registre->a == 'X') ||
      ((iContestacio != 0) && (tipus_registre->a == 'D')) ||
      ((tipus_registre->a == 'D') && ((iOpe == 112) || (iOpe == 114))))
   {
      escriure_salt_registre();
   }
...
#ifdef DEBUG
   printf("Acaba de generar 'lpszOrdres'\n"); getch();
#endif
   return 0;
}
